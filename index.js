...
loginIcon.onclick = function(){
    page.src = pageUrls[7] + '?username=' + window.sessionStorage.getItem('username')
}
...
const saveFileByself = async function (inputFile, folder) {
    ...
    try {
        ...
        const success = await window.electronAPI.saveFile({ name: file.name, data: fileData }, folder);
        if (success) {
            // 保存成功后的操作
            console.log('文件保存成功');
        } else {
